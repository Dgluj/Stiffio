...
\begin{titlepage}
	...
	{\large \textbf{Fecha de entrega:} 24/02/2026\par} 
...
\section{Objetivos}
    ...
    \begin{enumerate}
        ...
        Calcular la frecuencia cardíaca (HR) mediante la detección del intervalo entre pulsos consecutivos en la señal proximal. 
        \vspace{0.1cm}
        
        \item \textbf{Visualización de resultados mediante una aplicación web}\par 
        Mostrar en tiempo real los valores de PWV y HR calculados y graficar las señales fotopletismográficas (PPG) capturadas por los sensores proximales y distales. \vspace{0.1cm}
        
        \item \textbf{Comparación del valor obtenido con valores fisiológicos normales}\par 
        Indicar en la aplicación si el paciente se encuentra dentro de los rangos normales reportados en la literatura para su grupo etario.
        \vspace{0.1cm}
    ...
    \begin{enumerate}
        \item \textbf{Diseño y fabricación de clips y carcasa}\par 
        Diseñar y fabricar los clips de sujeción para los sensores proximal y distal, junto con la carcasa del dispositivo, asegurando una correcta integración mecánica, estabilidad durante la medición y ergonomía en el uso. 
        \vspace{0.1cm}

        \item \textbf{Gestión y almacenamiento de datos del paciente}\par 
        Implementar un sistema para registrar y guardar las mediciones obtenidas de cada paciente, permitiendo el seguimiento y comparación de resultados a lo largo del tiempo.
        \vspace{0.1cm}
...
\section{Materiales y Métodos}
    \subsection{Hardware}
        ...
        La unidad central del dispositivo está constituida por una placa ESP32, seleccionada debido a su elevada precisión temporal, característica imprescindible para medir con exactitud los intervalos entre señales fotopletismográficas y, de este modo, calcular correctamente el tiempo de tránsito del pulso. Además, este microcontrolador ofrece múltiples canales de comunicación I²C, lo que facilita la conexión simultánea de los periféricos digitales utilizados en el sistema.
        
        \vspace{0.1cm}

        Para la adquisición de las señales fotopletismográficas se emplearon dos sensores MAX30102, ubicados en posiciones anatómicas diferentes con el fin de registrar la componente proximal y distal del pulso. Ambos módulos se comunican con el ESP32 mediante el protocolo I²C. Sin embargo, cada sensor fue conectado a un conjunto distinto de pines SDA y SCL del microcontrolador, utilizando así dos buses I²C independientes. Esta configuración permite evitar conflictos de dirección y garantizar una lectura simultánea y estable de ambas señales. Asimismo, se integró una pantalla OLED que utiliza el mismo protocolo de comunicación para mostrar en tiempo real la información de las mediciones, junto con el estado de conexión de los sensores y la conectividad de red. Adicionalmente, se incorporó un LED indicador de encendido.

        \vspace{0.1cm}

        La alimentación del sistema se realiza mediante una batería de 9 V conectada a un regulador de tensión lineal, encargado de obtener una salida estable de 3.3 V. Esta tensión se utiliza para alimentar todos los componentes del circuito, incluyendo el microcontrolador.

        \vspace{0.1cm}

        En la Figura \ref{fig:Circuito} se presenta el esquema completo del circuito diseñado. 

        \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{images/Circuito.jpg}
        \caption{Esquema del circuito (las entradas de tensión se corresponden a la salida de alimentación del regulador).}
        \label{fig:Circuito}
        \end{figure}

         A partir de este diagrama se desarrolló una placa PCB de doble faz, en la cual se montaron todos los componentes del dispositivo. La elección de una PCB de dos capas permitió optimizar la distribución del cableado y ubicar estratégicamente cada componente de acuerdo con el diseño de la carcasa.
    

        

        
        \subsubsection{Diseño y Fabricación de Carcasa y Sensores}
        El diseño de la carcasa del dispositivo se realizó utilizando el software de diseño asistido por computadora \textit{SolidWorks}. El modelo tridimensional desarrollado se presenta en la Figura \ref{fig:Caja Solid}. En el Anexo se incluyen imágenes adicionales del modelo, donde se detallan con mayor precisión la geometría y las características constructivas de la carcasa.
        
        \vspace{0.1cm}

        El diseño fue concebido con el objetivo de alojar de manera segura los componentes electrónicos y, al mismo tiempo, facilitar la interacción del usuario con el dispositivo. En uno de los laterales de la carcasa se incorporó un orificio destinado al interruptor de encendido y apagado, mientras que en el panel trasero se dispusieron dos orificios para los conectores de los sensores.
        La tapa superior cuenta con una abertura rectangular que permite la correcta visualización de la pantalla, un orificio adyacente para el LED indicador de encendido y un segundo orificio de menor tamaño para permitir la salida del sonido generado por el buzzer. La tapa se fija al cuerpo de la carcasa mediante tornillos con cabeza Allen, los cuales atraviesan también la placa electrónica, asegurando su correcta sujeción. Esta configuración dificulta el acceso del usuario a los componentes internos, limitando la apertura del dispositivo únicamente a situaciones que requieran mantenimiento o servicio técnico.
        Por otro lado, en la parte inferior de la carcasa se incorporó un compartimiento independiente, aislado del área donde se encuentra la placa electrónica, destinado al alojamiento de las baterías. Este compartimiento incluye un portapilas para cuatro baterías AA y presenta un diseño de fácil acceso para el usuario mediante una tapa deslizante, la cual cuenta con un mecanismo de traba que genera un clic al cerrarse.

        \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{images/Caja Solid.png}
        \caption{Modelado de carcasa 3D.}
        \label{fig:Caja Solid}
        \end{figure}
        
        La carcasa fue fabricada mediante impresión 3D utilizando filamento PLA. La pieza final puede observarse en la Figura \ref{fig:Caja Impresa}.
        ...
        \label{fig:Caja Impresa}
        \end{figure}

        
        Para la conexión de los sensores al dispositivo, se eligieron conectores metálicos circulares de 4 pines. Estos incluyen una rosca de seguridad que garantiza una sujeción firme y previene desconexiones accidentales durante el uso. 
        
        Además, con el fin de guiar al usuario en su correcta conexión, se implementó un sistema de identificación por colores. Se colocó una marca del color correspondiente a cada sensor justo encima de su puerto, rojo para el sensor proximal y rosa para el distal, como se observa en la Figura \ref{fig:Colores Conectores}. 
        ...
        Los sensores MAX30102 se integraron en una funda de silicona para facilitar su colocación sobre el paciente y mejorar el contacto para una medición adecuada. La funda incorpora a sus lados dos elásticos con cierres de velcro, que permiten su ajuste seguro alrededor del cuello y la muñeca del paciente, adaptándose a la medida necesaria. El sensor en su funda de silicona con elásticos se muestra detalladamente en la Figura \ref{fig:Silicona}.

        \begin{figure}[H]
        \centering
        \includegraphics[width=0.7\textwidth]{images/Silicona.png}
        \caption{Fundas de silicona con sensor MAX30102 integrado}
        \label{fig:Silicona}
        \end{figure}
