...
\section{Introducción}
    ...
    La determinación de la PWV se basa en la medición del tiempo de tránsito del pulso (PTT) entre dos ubicaciones arteriales, generalmente un punto proximal y otro distal, como se muestra en la Figura~\ref{fig:calculo_pwv}, junto con la estimación de la distancia anatómica recorrida por la onda de presión. La velocidad de la onda de pulso se calcula a partir de la siguiente relación:
    ...
    Por otro lado, la oscilometría avanzada permite evaluar la rigidez arterial a partir de registros de presión obtenidos mediante un manguito inflable, similar al utilizado en la medición convencional de la presión arterial. Sistemas como \textit{Arteriograp} utilizan un único manguito colocado generalmente en el brazo, con el cual registran las oscilaciones de presión generadas por la onda de pulso incidente y su correspondiente onda reflejada. A partir del análisis temporal entre ambos componentes de la señal, estos dispositivos estiman de manera indirecta la velocidad de la onda de pulso central basándose en modelos hemodinámicos y supuestos teóricos acerca de la propagación y reflexión de la onda de pulso en el árbol arterial.
...
El presente proyecto se centra en el desarrollo de un dispositivo biomédico de bajo costo diseñado para la estimación no invasiva de la rigidez arterial, utilizando como indicador principal la velocidad de onda de pulso (PWV). 

\vspace{0.2cm}

El sistema emplea dos sensores ópticos infrarrojos para capturar la onda de pulso en dos puntos estratégicos del trayecto arterial. El sensor proximal se coloca a nivel de la arteria carótida en el cuello, mientras que el sensor distal va colocado sobre de la arteria radial en la muñeca. La selección de estos puntos anatómicos responde a criterios de ergonomía y calidad de señal. Ambas son zonas de fácil acceso para el operador, ofrecen comodidad al paciente y presentan una baja densidad capilar, lo que optimiza la lectura óptica. Desde el punto de vista fisiológico, la medición en el segmento carótido-radial permite evaluar un trayecto arterial significativo, proporcionando una estimación de la rigidez que no se limita exclusivamente a la vasculatura periférica, sino que incorpora componentes centrales del árbol arterial.

El principio de operación del dispositivo consiste en calcular el tiempo de tránsito del pulso (PTT) a partir del desfase temporal entre las señales proximal y distal. Combinando este valor con la distancia anatómica entre los sensores, el sistema computa la PWV. La distancia utilizada para el cálculo se estima a partir de la altura del paciente Adicionalmente, a partir de la señal del sensor distal, se obtiene la frecuencia cardíaca (HR) como variable adicional.
...
\section{Materiales y Métodos}
    ...
    \subsection{Hardware}
        ...
        \begin{figure}[H]
        \centering
        \includegraphics[width=0.65\textwidth]{images/Colores Puertos.png}
        \captionsetup{margin=1.5cm, justification=centering}
        ...
        \begin{figure}[H]
        \centering
        \includegraphics[width=0.45\textwidth]{images/Sensor.png}
        \captionsetup{margin=1.5cm, justification=centering}
    ...
    \subsection{Software}
        ...
        \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{images/DiagramaSoftwareMenu.png}
        \captionsetup{margin=1.5cm, justification=centering}
    ...
    \subsection{Interfaces de usuario}
        ...
        \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{images/Historial Interfaz.png}
    ...
    En esta sección se describe el enfoque metodológico adoptado para la evaluación del dispositivo, detallando las condiciones bajo las cuales se realizaron las mediciones y los criterios utilizados para su análisis.
        ...
        \begin{itemize}
            ...
            \item Fijar ambos sensores utilizando las bandas elásticas con velcro, asegurando contacto adecuado con la piel sin generar compresión excesiva.
            \item Verificar que ambos sensores esten colocados adecuadamente. El dispositivo indicará visualmente si alguno de los sensores no se encuentra haciendo buen contacto.
        \end{itemize}
        ...
        \begin{itemize}
            ...
            \item Una vez obtenido el resultado, presionar el botón “Guardar Medición” para almacenarlo en el sistema para su posterior análisis.
            \item En caso de observar señal pletismográfica inestable o CON artefactos por movimiento, reacomodar los sensores y repetir la medición.
        \end{itemize}
        ...
        Para determinar la precisión y la variabilidad intra-sujeto de las mediciones, se realizó un análisis de repetitibilidad. El estudio se llevó a cabo sobre dos sujetos diferentes, siguiendo estrictamente el protocolo de medición previamente establecido. 
        
        Los sujetos evaluados fueron dos voluntarios sanos, sin antecedentes de enfermedad cardiovascular ni factores de riesgo conocidos. Durante el procedimiento se mantuvieron constantes las condiciones ambientales, la postura corporal y el estado de reposo previo a cada adquisición, con el fin de minimizar posibles variaciones fisiológicas que pudieran influir en los resultados. Todas las mediciones correspondientes a cada sujeto se realizaron en una misma sesión, evitando cambios significativos en el estado hemodinámico.

        Para cada sujeto se analizaron dos escenarios experimentales. En primer lugar, se realizaron 10 mediciones consecutivas sin retirar ni modificar la posición de los sensores entre adquisiciones. Posteriormente, se efectuaron otras 10 mediciones consecutivas, retirando completamente y recolocando los sensores antes de cada adquisición. La comparación entre ambas condiciones tiene como objetivo discriminar qué proporción de la variabilidad total se atribuye al reposicionamiento de los sensores y a la intervención del operador, y qué parte corresponde a la variabilidad intrínseca del sistema electrónico y del algoritmo de procesamiento.
        ...
        Con el objetivo de evaluar la coherencia fisiológica de los valores obtenidos con el dispositivo desarrollado, se realizó una comparación de los mismos con los rangos de PWV reportados en la literatura. Para ello, se seleccionaron diez sujetos sanos distribuidos en dos grupos etarios diferenciados. El primer grupo estuvo conformado por cinco participantes con edades comprendidas entre 20 y 25 años, mientras que el segundo grupo incluyó cinco participantes entre 55 y 60 años. Ninguno de los sujetos presentaba antecedentes de enfermedad cardiovascular ni factores de riesgo conocidos. Al igual que en el análisis de repetitibilidad, todas las mediciones se realizaron siguiendo estrictamente el protocolo previamente descripto.
...
% RESULTADOS =======================================================================================================
\section{Resultados}
Los resultados correspondientes al análisis de repetitibilidad se presentan en la Tabla \ref{tab:rep_sin} y Tabla \ref{tab:rep_con}, donde se muestran las mediciones obtenidas bajo las dos condiciones experimentales: sin reposicionamiento y con reposicionamiento de sensores, respectivamente.

\begin{table}[H]
\centering
\caption{Mediciones de PWV y HR sin reposicionamiento de sensores}
\label{tab:rep_sin}
\begin{tabular}{c c c}
\hline
Medición & PWV (m/s) & HR (bpm) \\
\hline
1  & 8.0 & - 72\\
2  & 8.3 & - 74\\
3  & 8.4 & - 73\\
...
\begin{table}[H]
\centering
\caption{Mediciones de PWV y HR con reposicionamiento de sensores}
\label{tab:rep_con}
\begin{tabular}{c c c}
\hline
Medición & PWV (m/s) & HR (bpm) \\
...
\hline
Media & 2.74 & - \\
Desvío estándar & 0.44 & - \\
CV (\%) & 15.97\% & - \\
\hline
\end{tabular}
\end{table}

Bajo la condición sin reposicionamiento, el coeficiente de variación obtenido fue de X\%, indicando .... En la condición con reposicionamiento de sensores, el coeficiente de variación fue de Y\%, evidenciando un aumento del en la variabilidad asociada a la recolocación manual.

\vspace{0.4cm}

Los valores individuales de velocidad de onda de pulso (PWV) y frecuencia cardíaca (HR) obtenidos en las mediciones realizadas a los diferentes sujetos se presentan en la Tabla \ref{tab:mediciones}. 

\begin{table}[H]
\centering
\caption{Valores individuales de PWV y frecuencia cardíaca (HR) obtenidos medición diferentes sujetos}
\label{tab:mediciones}
\begin{tabular}{c c c c}
\hline
Grupo & Sujeto & PWV (m/s) & HR (lpm) \\
\hline
\multirow{5}{*}{20–25 años} 
& S1 &  &  \\
& S2 &  &  \\
& S3 &  &  \\
& S4 &  &  \\
...
La media y el desvío estándar correspondientes a cada grupo etario se presentan de forma resumida en la Tabla \ref{tab:mediciones_media}.

\begin{table}[H]
\centering
\caption{Media y desvío estándar de PWV y HR por grupo etario}
\label{tab:mediciones_media}
\begin{tabular}{c c c c c}
\hline
Grupo & PWV media (m/s) & PWV DE & HR media (lpm) & HR DE \\
\hline
20–25 años &  &  &  &  \\
55–60 años &  &  &  &  \\
\hline
...
Para el grupo de 20 a 25 años, la media obtenida fue de X m/s, en comparación con el valor de referencia reportado en la literatura de Y m/s. El análisis mediante prueba t de Student arrojó un valor de ... No se observaron diferencias estadísticamente significativas respecto al valor de referencia (p > 0,05).

Para el grupo de 55 a60 años, la media fue de ... m/s, con un valor de p = ...
...
Se observó una marcada dependencia de la correcta colocación de los sensores. Dado que el sistema se basa en la detección de señales pletismográficas en sitios arteriales específicos, pequeñas variaciones en la posición, en la presión ejercida o en la alineación respecto al eje arterial pueden modificar la morfología de la señal y afectar la determinación del tiempo de tránsito. Esta característica introduce una componente operador-dependiente que debe considerarse en el entrenamiento del usuario. Como posible línea de mejora, podría desarrollarse un sistema de posicionamiento asistido o un algoritmo automático de evaluación de calidad de señal que proporcione retroalimentación en tiempo real.

Asimismo, el dispositivo mostró sensibilidad al ruido por movimiento. Artefactos generados por respiraciones profundas, deglución o contracciones musculares pueden alterar la estabilidad de la señal y comprometer la detección precisa de los puntos característicos utilizados para el cálculo de la velocidad de onda de pulso. Esto pone de manifiesto la necesidad de optimizar los métodos de filtrado y los algoritmos de detección de eventos no fisiológicos, minimizando la probabilidad de que estos afecten la estimación final.
...
\subsection{Comparación con dispositivos del mercado}
...
El presente trabajo demostró la factibilidad técnica de estimar la velocidad de onda de pulso mediante un sistema basado en la medición del tiempo de tránsito entre señales pletismográficas periféricas. El dispositivo desarrollado logró obtener mediciones fisiológicamente coherentes en condiciones controladas, validando el enfoque conceptual adoptado. Si bien se identificaron limitaciones metodológicas y técnicas, los resultados obtenidos confirman la viabilidad del sistema.
...
\renewcommand{\refname}{} % evita el título automático
